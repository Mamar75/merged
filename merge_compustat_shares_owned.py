# python '20_merge/merge_compustat_shares_owned.py'
# =============================================================================
...
crsp_data = spark.read.csv('17_data_new/compustat_daily_data.csv', header=True, inferSchema=True)
crsp_data = crsp_data.withColumn("datadate", to_date(crsp_data["datadate"], "yyyyMMdd"))

# Add cusip8 and cusip6 from cusip
crsp_data = crsp_data.withColumn('cusip8', F.expr("substring(cusip, 1, 8)"))
crsp_data = crsp_data.withColumn('cusip6', F.expr("substring(cusip, 1, 6)"))
...
final_data_1 = merged_data.join(
    crsp_data,
    (merged_data.date_sample == crsp_data.datadate) & (merged_data.cusips_sliced == crsp_data.cusip8),
    'inner'
)

# Drop redundant columns
final_data_1 = final_data_1.drop('datadate')
...
final_data_2 = merged_data_anti.join(
    crsp_data,
    (merged_data_anti.date_sample == crsp_data.datadate) & (merged_data_anti.cusips_sliced == crsp_data.cusip6),
    'inner'
)

# Drop redundant columns
final_data_2 = final_data_2.drop('datadate')
...
final_data = final_data.join(
    crsp_data,
    (final_data.date_sample == crsp_data.datadate_o) & (final_data.cik_owner == crsp_data.cik_o),
    'inner'
)

# Drop redundant columns
final_data = final_data.drop('datadate_o')
...
final_data.write.mode('overwrite').csv('20_merge/final_data_filled', header=True)
